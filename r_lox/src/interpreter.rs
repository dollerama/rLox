...
impl Interpreter {
    ...
    fn assign_helper(current_val : RuntimeError<Option<Literal>>, value : Option<Literal>, expr : &Assign) -> RuntimeError<Option<Literal>> {
        match current_val {
            Ok(current) => {
                        ...
                        match v {
                            TokenType::Plus => {
                                match (current, value.clone()) {
                                    ...
                                    _ => {
                                        Err((expr.name.clone(), "Operands must be Numbers Or Strings.".to_string()))
                                    }
                                }
                            },
                            TokenType::Minus => {
                                match (current, value.clone()) {
                                    ...
                                    _ => {
                                        Err((expr.name.clone(), "Operands must be Numbers.".to_string()))
                                    }
                                }
                            },
                            TokenType::Slash => {
                            ...
                            _ => Err((expr.name.clone(), "Invalid assign".to_string()))
                        }
                    }
                    None => {
                        match (current, value.clone()) {
...
impl ExprVisitor for Interpreter {
    ...
    fn visit_assign_expr(&mut self, expr : &Assign) -> RuntimeError<Option<Literal>> {
        ...
        let new_value = Self::assign_helper(self.environment.get(expr.name.clone()), value.clone(), expr)?;
    ...
    fn visit_call_expr(&mut self, expr : &Call) -> RuntimeError<Option<Literal>> {
        ...
        let callee_token = if let Some(v) = &expr.callee.as_any().downcast_ref::<VarExpr>() {
            ...
            return Err((Token::new(TokenType::Nil, "nil", None, -1), "Unexpected callee.".to_string()));
        };
